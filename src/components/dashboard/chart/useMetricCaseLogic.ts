...
if (metricType === "Ground Contact Time (s)") { value = pick(["time_to_takeoff", "contact_time"]); yAxisLabel = "Ground Contact Time (s)"; }
break;
// ── VALD-specific test names (correct VALD Hub terminology) ──
case "Single Leg Jump":
// SLJ in VALD — single leg reactive jump, classified as pogo for metric extraction
if (metricType === "RSI") { value = pick(["avg_rsi", "rsi", "avg_modified_rsi"]); yAxisLabel = "RSI (m/s)"; }
if (metricType === "Jump Height (cm)") value = pick(["avg_jump_height", "jump_height_ft", "jump_height"]);
if (metricType === "Contact Time") { value = pick(["avg_contact_time", "contact_time"]); yAxisLabel = "Contact Time (ms)"; }
if (metricType === "Flight Time") { value = pick(["avg_flight_time", "flight_time"]); yAxisLabel = "Flight Time (ms)"; }
break;
case "Hop Test":
case "Standing Long Jump":
case "Horizontal Jump":
// HJ in VALD — bilateral reactive hop, classified as pogo
if (metricType === "RSI") { value = pick(["avg_rsi", "rsi"]); yAxisLabel = "RSI (m/s)"; }
if (metricType === "Contact Time") { value = pick(["avg_contact_time", "contact_time"]); yAxisLabel = "Contact Time (ms)"; }
if (metricType === "Flight Time") { value = pick(["avg_flight_time", "flight_time"]); yAxisLabel = "Flight Time (ms)"; }
if (metricType === "Jump Height (cm)") value = pick(["avg_jump_height", "jump_height"]);
break;
case "Single Leg Hop Test":
case "Single Leg Hop for Distance":
// SLHJ in VALD — single leg hop, classified as pogo
if (metricType === "RSI") { value = pick(["avg_rsi", "rsi"]); yAxisLabel = "RSI (m/s)"; }
if (metricType === "Contact Time") { value = pick(["avg_contact_time", "contact_time"]); yAxisLabel = "Contact Time (ms)"; }
if (metricType === "Flight Time") { value = pick(["avg_flight_time", "flight_time"]); yAxisLabel = "Flight Time (ms)"; }
if (metricType === "Jump Height (cm)") value = pick(["avg_jump_height", "jump_height"]);
