...
export const ReportFilters = ({ 
  ...
  const handleTestNameChange = (value: string) => {
    setFilters(prev => ({ ...prev, testName: value, metricType: "" }));
    onTestSelect(value);
  };
  ...
  const handleAthleteToggle = (athleteName: string) => {
    const newAthletes = filters.selectedAthletes.includes(athleteName)
      ? filters.selectedAthletes.filter(a => a !== athleteName)
      : [...filters.selectedAthletes, athleteName];
    setFilters(prev => ({ ...prev, selectedAthletes: newAthletes }));
  };
  ...
  const handleDateToggle = (date: string) => {
    ...
    setFilters(prev => ({ ...prev, testDates: newDates }));
  };
          ...
          <div className="flex flex-wrap gap-2">
            {uniqueTeams.map(team => (
              <Button
                ...
                className={selectedTeams.includes(team) ? "bg-teal-600 text-white" : ""}
              >
                {team}
              </Button>
            ))}
          ...
          <div>
            ...
            <div className="max-h-32 overflow-y-auto border rounded p-2 bg-white">
              {uniqueAthletes.map(athlete => (
                <div key={athlete} className="flex items-center space-x-2 mb-1">
                  <input
                    type="checkbox"
                    id={`athlete-${athlete}`}
                    checked={filters.selectedAthletes.includes(athlete)}
                    onChange={() => handleAthleteToggle(athlete)}
          ...
          <div>
            ...
            <div className="max-h-32 overflow-y-auto border rounded p-2 bg-white">
              {uniqueTestDates.map(date => (
                <div key={date} className="flex items-center space-x-2 mb-1">
                  <input
                    type="checkbox"
                    id={`date-${date}`}
                    checked={filters.testDates.includes(date)}
