...
export const useAthletesNeedingAttention = (teamId?: string | null) => {
    return useQuery({
        ...
        // Scope the test lookup to this team's name too. test_data has no
        // reliable team_id on every row (legacy API-synced rows can have it
        // null), but team_name is NOT NULL at the schema level, so matching
        // on it avoids pulling in a different team's same-named athlete's
        // test record and silently showing the wrong "last tested" date.
        ...
        if (team?.name) {
            testsQuery = testsQuery.eq("team_name", team.name);
        }
...
export const useAthleteProgress = (userId?: string | null, teamId?: string | null) => {
    return useQuery({
        queryKey: ["home-athlete-progress", userId, teamId],
        ...
        // Scope the test_data lookups to this athlete's own team too. Without
        // this, a same-named athlete on a different team could silently feed
        // into this athlete's own testsThisMonth/testsLastMonth/recentTests —
        // same bug class already fixed in useAthletesNeedingAttention above and
        // in ClientToday.tsx/ClientReports.tsx/useClientMetrics.ts (PR #34/#37/
        // #38/#39/#40). Falls back to the old unscoped query if the team lookup
        // comes back empty, so this can only get more accurate, never worse.
        ...
        if (teamName) {
            thisMonthQuery = thisMonthQuery.eq("team_name", teamName);
            lastMonthQuery = lastMonthQuery.eq("team_name", teamName);
            recentQuery = recentQuery.eq("team_name", teamName);
        }
