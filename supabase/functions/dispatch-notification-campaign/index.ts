...
Deno.serve(async (req) => {
  if (req.method === "OPTIONS") return new Response("ok", { headers: corsHeaders });
           ...
           try {
             // Require a real, authenticated super admin. This endpoint used to have
  // no check at all beyond Supabase's default "some validly-signed JWT"
  // requirement, which the public anon key satisfies with no login. It
  // dispatches a queued platform notification campaign - real emails to
  // every org owner, in-app notifications, and webhook POSTs to external
  // URLs - so an anon-key-only caller triggering it early was a real abuse
  // path, not just a read. Gated the same way cc-retry-sync gates its
  // super-admin-only actions: a real user via auth.getUser(), then the
  // is_super_admin() RPC. Both frontend callers (Notifications.tsx,
  // NotificationCampaignDrawer.tsx) already send the logged-in user's own
  // session, so this is a no-op for legitimate use.
