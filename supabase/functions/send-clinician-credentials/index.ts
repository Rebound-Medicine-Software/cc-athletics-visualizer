...
const handler = async (req: Request): Promise<Response> => {
    ...
    try {
      ...
      //
      // SendPulse is a secondary/legacy notification path here — by this point
      // the account has already been created successfully above. Both callers
      // of this function (Setup.tsx's onboarding wizard, and
      // StaffCredentialsTab.tsx's "Add Staff User" flow) also fire the
      // NotificationAPI-based send-practitioner-invite function as the real
      // credentials-delivery email. A SendPulse failure here must not report
      // the whole request as failed — that used to abort account-creation
      // flows in the frontend (StaffCredentialsTab.tsx throws on any error
      // from this function) and skip the working send-practitioner-invite
      // call entirely, even though the account itself was created fine.
      ...
      if (!sendpulseUserId || !sendpulseSecret) {
              console.error("SendPulse credentials are not configured — skipping SendPulse email. Account creation still succeeds.");
      } else {
              ...
              try {
                ...
                const emailResult = await emailResponse.json();
                        ...
                        // Non-fatal: the account was already created above, and both callers
                // have their own working NotificationAPI-based invite email as the
                // real delivery path. Log and continue instead of failing the request.
                console.error("Error sending email via SendPulse (non-fatal):", error);
              }
      }
      ...
      return new Response(
              JSON.stringify({
                        success: true,
                        message: emailSent ? "Clinician account created and credentials sent" : "Clinician account created (SendPulse email not sent)",
                        user: userData.user,
                        emailSent: emailSent
              }),
        {
