...
const handler = async (req: Request): Promise<Response> => {
  ...
  // every "forgotten email" request silently vanished. There's no existing
  ...
  // (visible in the Control Centre) as a warning-level event so Josh/super admins can
  // actually see and action it. Swap this for a real Pingram-templated email once that
  // template exists.
  await logActivity({
    eventType: 'account_recovery_requested_email_only',
    ...
    metadata: {
      ...
      note: 'Forgotten email only. Verify identity before assisting - HIPAA-compliant platform.',
    },
  });
