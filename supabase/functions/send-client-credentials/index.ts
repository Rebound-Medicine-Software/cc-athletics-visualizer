...
const handler = async (req: Request): Promise<Response> => {
  ...
  // handle_new_user() only fills profiles.full_name from
  // raw_user_meta_data.full_name / .name — this function previously only
  // ever sent first_name/last_name, so it never populated full_name for
  // any client/athlete account (unlike send-clinician-credentials, which
  // sends full_name directly). That silently broke the athlete-facing
  // progress view (useAthleteProgress), which falls back to matching
  // test_data rows by the profile's email when full_name is missing —
  // and test_data stores real athlete names, not emails, so that lookup
  ...
  // Explicitly set full_name for both branches above — the
  // handle_new_user() trigger only runs on INSERT, so it never fires for
  // the existingUser/updateUserById path, and we don't want to rely on
  // user_metadata parsing alone for the new-user path either.
  if (clientUserId && fullName) {
    const { error: nameErr } = await supabase
    .from('profiles')
    .update({ full_name: fullName })
    .eq('user_id', clientUserId);
    if (nameErr) {
      console.warn('Failed to set profile full_name:', nameErr);
    }
  }
