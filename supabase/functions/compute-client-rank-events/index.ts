...
serve(async (req) => {
    if (req.method === 'OPTIONS') return new Response('ok', { headers: corsHeaders })
      ...
      // 1b. Load per-team retest interval (fall back to default) and team name
      // (the team name is also used below to scope an athlete's own PB/last-test
      // lookups so a same-named athlete on a different team can't corrupt them).
      ...
      for (const ath of linked) {
              // Scope this athlete's OWN data (their PB baseline, last-test date) to
                // their own team by name — athlete_name alone can collide across teams,
                // same fix pattern as useHomeRoleData.ts's useAthletesNeedingAttention.
                // Falls back to an unscoped name match if we don't have a team name on
                // file, so this can only make results more accurate, never worse. This
                // does NOT touch the peer-comparison ranking scopes below (club/sport/
                // region/global) — those are deliberately cross-team by design.
                const athTeamName = ath.team_id ? teamNameById.get(ath.team_id) ?? null : null
              const matchesAthlete = (r: any) =>
                        r.athlete_name === ath.name && (!athTeamName || r.team_name === athTeamName)
